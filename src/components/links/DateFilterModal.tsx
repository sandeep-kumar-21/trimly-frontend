...
export const DateFilterModal: React.FC<DateFilterModalProps> = ({
  ...
  return createPortal(
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 select-none">
      ...
      {/* Modal Container */}
      <div className="relative z-10 w-full max-w-md rounded-xl bg-white p-6 sm:p-7 shadow-2xl border border-slate-200/80 dark:bg-slate-900 dark:border-slate-800 space-y-5 animate-in zoom-in-95 duration-200">
        {/* Header */}
        ...
        <div className="space-y-5">
          ...
          <div className="grid grid-cols-2 gap-3">
            <input
              ...
              className="h-10 w-full rounded-lg border border-slate-200 bg-slate-100/70 px-3.5 text-sm font-semibold text-[#273144] text-center dark:border-slate-800 dark:bg-slate-800 dark:text-slate-200"
            />
            <input
              type="text"
              readOnly
          ...
          <div className="space-y-3 pt-1">
            ...
            <div className="space-y-2">
              {/* Weekday Headers */}
              <div className="grid grid-cols-7 text-center text-sm font-semibold text-[#526281] dark:text-slate-400">
                <span>Sun</span>
                <span>Mon</span>
                <span>Tue</span>
                <span>Wed</span>
                ...
                {calendarCells.map((cell, idx) => {
                  ...
                  if (isFuture) {
                    return (
                      <span
                        key={idx}
                        className="py-1.5 text-sm text-slate-300 dark:text-slate-700 pointer-events-none font-normal"
                      >
                        {cell.dayNumber}
                      </span>
                    );
                  ...
                  if (!cell.isCurrentMonth) {
                    return (
                      <button
                        ...
                        className={`py-1.5 text-sm font-semibold transition-colors cursor-pointer ${
                          inRange
          ...
          <div className="rounded-xl bg-[#f4f6f8] p-3.5 dark:bg-slate-800/60 flex flex-wrap gap-2">
            {presets.map((preset) => (
              <button
                key={preset}
                type="button"
                onClick={() => handlePresetSelect(preset)}
                className={`rounded-lg px-3 py-1.5 text-sm font-semibold shadow-2xs transition-colors cursor-pointer ${
